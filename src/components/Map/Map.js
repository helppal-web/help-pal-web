...
export default function MapComponent({ showModal, markers }) {
    ...
    return ( <
        ...
        <
        Button className = "mx-5 rounded-circle new-call"
        variant = "helppal"
        onClick = {
            () => setShowRequestModal(true) } > + < /Button> <
        Button variant = "none"
        className = "mx-2"
        onClick = {
            () => setFiltersModal(true) } >
        <
        FilterListIcon className = "filter-icon" > < /FilterListIcon> { t('Filters') } <
        /Button> <
        /div>
        ...
        zoom = { state.zoom } >
        <
        TileLayer attribution = 'Map data &copy; <a href="https://www.openstreetmap.org/">OpenStreetMap</a> contributors, <a href="https://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, Imagery © <a href="https://www.mapbox.com/">Mapbox</a>'
        ...
        /> {
            markers.length ? markers.map((marker, index) =>
                ...
                Marker icon = { myIcon }
                key = { index }
                position = { marker.position } > { marker.content } <
                /Marker>
            ) : ''
        } <
        /Map>
        ...
        <
        Modal centered show = { showRequestModal }
        onHide = { hideRequestModal }
        ...
        Modal.Title >
        <
        FilterListIcon className = "filter-icon" / > { t('Filter By') } <
        /Modal.Title> <
        /Modal.Header> <
        Modal.Body >
        <
